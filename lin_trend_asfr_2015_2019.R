################################################################################
## Age-standardised fertility rates (ASFR) 2011-2023
## Baseline = country-specific linear trend fitted on 2015-2019,
##            and % deviation in 2020-2023
################################################################################
...
library(kableExtra)
library(scales)
library(broom)      # needed for augment() / tidy()
library(htmltools)
library(ggplot2)
...
## 4 ────────────────────────────────────────────────────────────────────────────
##  BASELINE = **linear trend** of 2015-2019 for each country
##  ---------------------------------------------------------
##  • Fit ASFR ~ year on 2015-2019.
##  • Predict for every year present in the ASFR data frame.
##    (This extrapolates modestly backwards to 2011 and forwards to 2023.)
trend_pred <- asfr %>%
  group_by(country) %>%
  group_modify(~{
    fit <- lm(ASFR ~ year, data = filter(.x, year %in% 2015:2019))
    tibble(year       = .x$year,
           pred_ASFR  = predict(fit, newdata = .x))
  }) %>%
  ungroup()

##  Attach baseline to every year; compute % deviation
asfr_pred <- asfr %>%
  left_join(trend_pred, by = c("country", "year")) %>%
  mutate(dev_pct = (ASFR - pred_ASFR) / pred_ASFR * 100)

## 5 ────────────────────────────────────────────────────────────────────────────
##  Table (same colour-band logic: negative = red, positive = blue)
dev_tbl_num <- asfr_pred %>%
...
tbl_html <- dev_tbl_col %>%
  kbl(format = "html", escape = FALSE,
      caption = "Percent deviation of ASFR from the 2015-2019 linear-trend baseline  
...
## 6 ────────────────────────────────────────────────────────────────────────────
##  Plot: observed ASFR, 2015-2019 linear-trend baseline, % deviation label
ggplot(asfr_pred, aes(year, ASFR)) +
  geom_line(linewidth = 0.7, colour = "steelblue") +
  geom_line(aes(y = pred_ASFR),                 # <<-- variable baseline
...
labs(
  title    = "Age-standardised fertility rates vs. 2015-2019 linear-trend baseline",
  subtitle = "Dashed line = fitted ASFR trend (2015-2019); labels = % deviation",
  x = NULL,
